<template>
         ...
         <div class="card p-3 border-blue mt-3"> 
                    <div class="comment d-flex justify-content-between mt-2" v-for="comment in comments" v-bind:key="comment.id" :comment="comment">
                        <div class="d-flex flex-row">
                            <h6 class="mb-0">{{comment.firstName}} {{comment.lastName}} </h6> <span class="date">{{post.createdAt}} </span>
                        </div>
                        <div class="com" >
                            <p class="content">{{post.comment}}</p>
                        </div>
                    </div>
        </div> 
</template>
